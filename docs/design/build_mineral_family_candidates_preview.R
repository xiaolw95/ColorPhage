...
if (!requireNamespace("ggplot2", quietly = TRUE)) {
  stop("Package `ggplot2` is required to build the mineral candidate preview.", call. = FALSE)
}
...
palettes <- list(
  mineral_slate = list(
    ...
    role = "石板蓝灰主线：适合临床亚组、方法比较、模型 benchmark。",
    colors = c("#26384A", "#536879", "#7F95A3", "#AFC0C8", "#D7D2C3", "#B8A27A", "#A46F5E", "#6E4F63")
  ),
  mineral_quartz = list(
    display_name = "Mineral Quartz",
    role = "通透石英层：适合低刺激主文统计图、组学分层和长期阅读场景。",
    colors = c("#415A66", "#6F8790", "#9EB0B4", "#D8DED9", "#C9BEA5", "#9E8E70", "#C48B73", "#7C6B82")
  ),
  mineral_oxide = list(
    display_name = "Mineral Oxide",
    role = "氧化铁锚点：适合疾病组、干预组、关键模块等需要突出主结果的图。",
    colors = c("#2B3F4A", "#4E6B73", "#789097", "#B5C1BE", "#D4C6A8", "#B9825D", "#A9574E", "#5E4A5D")
  )
)

design_dir <- file.path(root_dir, "docs", "design")
figure_dir <- file.path(design_dir, "mineral_family_preview_figures")
...
theme_preview <- function(base_size = 11) {
  ggplot2::theme_minimal(base_size = base_size) +
    ggplot2::theme(
      ...
      panel.grid.major = ggplot2::element_line(colour = "#e2ded3", linewidth = 0.35),
...
bar_plot <- function(colors) {
  values <- c(9.1, 11.8, 8.6, 10.4, 12.7, 9.8, 13.4, 10.9)
  df <- data.frame(
...
make_plot_files <- function(id, colors) {
  ...
  vapply(names(plots), function(kind) {
    ...
    file.path("mineral_family_preview_figures", basename(out))
  }, character(1))
}
  ...
  sprintf(
    '<section class="palette-section">
      <div class="palette-head">
        <div>
          <p class="eyebrow">advanced family / mineral</p>
          <h2>%s <span>%s</span></h2>
          <p>%s</p>
        </div>
      </div>
...
<head>
  ...
  <title>ColorPhage Mineral Family Candidates</title>
  <style>
    :root {
      --bg: #f0eee8;
      --panel: #fffdf8;
    ...
    * { box-sizing: border-box; }
    body {
      ...
      color: var(--ink);
      background:
        radial-gradient(circle at 10%% 10%%, rgba(83,104,121,.20), transparent 27%%),
        radial-gradient(circle at 88%% 6%%, rgba(164,111,94,.15), transparent 23%%),
        radial-gradient(circle at 78%% 88%%, rgba(127,149,163,.18), transparent 25%%),
        var(--bg);
      line-height: 1.55;
    }
    ...
    .hero, .palette-section {
      ...
      border-radius: 30px;
      box-shadow: 0 22px 54px rgba(60,70,82,.10);
    }
...
<body>
  <main class="wrap">
    <section class="hero">
      <h1>ColorPhage Mineral Family Candidates</h1>
      <p>这是第二个进阶 family 的候选预览。Mineral 走蓝灰、石板、矿物青、冷砂和少量铁锈锚点路线，目标是临床、方法学、模型比较和机制统计图中的冷静高级感。</p>
      <div class="mode-strip">
        <span class="pill">advanced family: mineral</span>
    ...
    %s
    <p class="footer">Generated by docs/design/build_mineral_family_candidates_preview.R</p>
  </main>
</body>
</html>',
  sections
...
out_file <- file.path(design_dir, "mineral_family_candidates_preview.html")
