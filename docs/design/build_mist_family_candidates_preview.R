...
if (!requireNamespace("ggplot2", quietly = TRUE)) {
  stop("Package `ggplot2` is required to build the mist candidate preview.", call. = FALSE)
}
...
palettes <- list(
  mist_cloud = list(
    ...
    role = "云层与浅雾：适合主文统计图、补充材料趋势图和轻量分层图。",
    colors = c("#8CA0B3", "#B3C0C9", "#D8E0DE", "#E8E5DE", "#E0D1C9", "#C9B7C4", "#A8B2C7", "#CFC6D8")
  ),
  mist_lilac = list(
    display_name = "Mist Lilac",
    role = "淡紫与雾蓝：适合多时间点变化图、温和细胞亚群比较和低刺激主文图。",
    colors = c("#9EA6C4", "#C0C6D8", "#DDE2E8", "#E9E6E1", "#D9CFD4", "#C7B8C8", "#B6C6D4", "#E0D7E5")
  ),
  mist_shell = list(
    display_name = "Mist Shell",
    role = "贝壳白与雾灰绿：适合机制模块图、综述补充图和长期阅读统计图。",
    colors = c("#95A7A0", "#B9C7C1", "#DDE3DE", "#EAE2D7", "#D8C7BC", "#C6B6AA", "#B8C5D1", "#D9D1DA")
  )
)

design_dir <- file.path(root_dir, "docs", "design")
figure_dir <- file.path(design_dir, "mist_family_preview_figures")
...
theme_preview <- function(base_size = 11) {
  ggplot2::theme_minimal(base_size = base_size) +
    ggplot2::theme(
      text = ggplot2::element_text(family = "sans", colour = "#3b4046"),
      plot.background = ggplot2::element_rect(fill = "#fffdf8", colour = NA),
      panel.background = ggplot2::element_rect(fill = "#fffdf8", colour = NA),
      panel.grid.major = ggplot2::element_line(colour = "#e4e2dc", linewidth = 0.35),
      panel.grid.minor = ggplot2::element_blank(),
      axis.title = ggplot2::element_blank(),
      axis.text = ggplot2::element_text(colour = "#7b7d82", size = 9),
      legend.position = "none",
      plot.margin = ggplot2::margin(10, 12, 10, 12)
    )
}

manual_color <- function(colors) ggplot2::scale_colour_manual(values = colors)
manual_fill <- function(colors) ggplot2::scale_fill_manual(values = colors)
...
bar_plot <- function(colors) {
  values <- c(8.6, 10.9, 9.1, 11.8, 10.7, 8.4, 12.9, 10.2)
  df <- data.frame(group = factor(paste0("G", seq_along(values))), value = values)
...
scatter_plot <- function(colors) {
  set.seed(1212)
  group <- factor(rep(paste0("C", 1:8), each = 14))
  centers <- data.frame(
...
line_plot <- function(colors) {
  ...
  group <- factor(rep(paste0("L", 1:8), each = 7))
...
umap_plot <- function(colors) {
  set.seed(1218)
  group <- factor(rep(paste0("T", 1:8), each = 70))
...
make_plot_files <- function(id, colors) {
  plots <- list(bar = bar_plot(colors), scatter = scatter_plot(colors), line = line_plot(colors), umap = umap_plot(colors))
  vapply(names(plots), function(kind) {
    out <- file.path(figure_dir, sprintf("%s_%s.svg", id, kind))
    save_svg_plot(plots[[kind]], out)
    file.path("mist_family_preview_figures", basename(out))
  }, character(1))
}

swatch_cards <- function(colors) paste(sprintf('<div class="swatch-card"><div class="chip" style="background:%s"></div><code>%s</code></div>', colors, colors), collapse = "\n")
plot_card <- function(title, src) sprintf('<div class="preview-card"><h3>%s</h3><img src="%s" alt="%s"></div>', escape_html(title), escape_html(src), escape_html(title))

palette_section <- function(id) {
  meta <- palettes[[id]]
  files <- make_plot_files(id, meta$colors)
  cards <- paste(plot_card("Fill / bar", files[["bar"]]), plot_card("Colour / scatter", files[["scatter"]]), plot_card("Colour / line", files[["line"]]), plot_card("UMAP-like", files[["umap"]]), sep = "\n")
  sprintf('<section class="palette-section"><div class="palette-head"><div><p class="eyebrow">advanced family / mist</p><h2>%s <span>%s</span></h2><p>%s</p></div></div><div class="swatches">%s</div><div class="preview-grid">%s</div></section>', escape_html(meta$display_name), escape_html(id), escape_html(meta$role), swatch_cards(meta$colors), cards)
}
...
<head>
  ...
  <title>ColorPhage Mist Family Candidates</title>
  <style>
    :root { --bg: #f5f4f1; --panel: #fffdf8; --ink: #3b4046; --muted: #7b7d82; --line: #e2e0da; }
    * { box-sizing: border-box; }
    body { margin: 0; font-family: "Segoe UI", "PingFang SC", "Microsoft YaHei", sans-serif; color: var(--ink); background: radial-gradient(circle at 10%% 10%%, rgba(158,166,196,.16), transparent 26%%), radial-gradient(circle at 86%% 6%%, rgba(232,229,222,.34), transparent 24%%), radial-gradient(circle at 78%% 88%%, rgba(201,183,196,.18), transparent 24%%), var(--bg); line-height: 1.55; }
    .wrap { width: min(1280px, calc(100%% - 48px)); margin: 36px auto 72px; }
    .hero, .palette-section { background: rgba(255,253,248,.94); border: 1px solid var(--line); border-radius: 30px; box-shadow: 0 22px 54px rgba(90,92,98,.08); }
    ...
    .pill { border: 1px solid var(--line); border-radius: 999px; padding: 8px 14px; background: #faf8f3; color: var(--muted); font-size: 14px; }
    ...
    h2 span { font-size: 14px; color: var(--muted); font-weight: 500; border: 1px solid var(--line); border-radius: 999px; padding: 5px 10px; vertical-align: middle; margin-left: 8px; }
    ...
    .swatch-card { border: 1px solid var(--line); background: #fff; border-radius: 18px; overflow: hidden; box-shadow: 0 8px 18px rgba(50,60,70,.05); }
    .chip { height: 98px; }
    .swatch-card code { display: block; padding: 10px 12px 12px; font-size: 12px; color: var(--muted); background: #faf8f3; }
    .preview-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 14px; }
    .preview-card { border: 1px solid var(--line); background: linear-gradient(180deg, #fff, #faf8f3); border-radius: 22px; padding: 16px 16px 12px; overflow: hidden; }
    .preview-card h3 { font-size: 15px; color: var(--muted); margin-bottom: 10px; }
    .preview-card img { width: 100%%; height: auto; display: block; border-radius: 14px; background: #fffdf8; }
...
<body>
  <main class="wrap">
    <section class="hero">
      <h1>ColorPhage Mist Family Candidates</h1>
      <p>这是第六个进阶 family 的候选预览。Mist 用低冲突雾蓝、浅灰绿、淡紫和柔和沙色构成轻盈、长期阅读友好的科研配色，目标是与 soft 拉开层次而不是简单重复。</p>
      <div class="mode-strip">
        <span class="pill">advanced family: mist</span>
    ...
    %s
    <p class="footer">Generated by docs/design/build_mist_family_candidates_preview.R</p>
  </main>
</body>
</html>',
  sections
...
out_file <- file.path(design_dir, "mist_family_candidates_preview.html")
