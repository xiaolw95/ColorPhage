...
if (!requireNamespace("ggplot2", quietly = TRUE)) {
  stop("Package `ggplot2` is required to build the ocean candidate preview.", call. = FALSE)
}
...
palettes <- list(
  ocean_current = list(
    ...
    role = "海流主线：适合组学样本分层、多来源样本比较和清洁主文散点图。",
    colors = c("#1F4E5F", "#337A84", "#62A5A7", "#A7CED1", "#D8E7E4", "#7FA6C2", "#C99A6B", "#B6635C")
  ),
  ocean_glass = list(
    display_name = "Ocean Glass",
    role = "海玻璃层：适合低刺激主文统计图、临床队列分层和长期阅读场景。",
    colors = c("#3E6674", "#6D94A0", "#9DB9C0", "#D4E1DE", "#B7CDC7", "#86A9B8", "#D6B48A", "#A87573")
  ),
  ocean_depth = list(
    display_name = "Ocean Depth",
    role = "深海层次：适合单细胞聚类、空间转录组分区和多亚型结果图。",
    colors = c("#123B4A", "#245E70", "#3F8292", "#7EB1B8", "#BFD8D6", "#5F7895", "#9B6E7B", "#D08965")
  )
)

design_dir <- file.path(root_dir, "docs", "design")
figure_dir <- file.path(design_dir, "ocean_family_preview_figures")
...
theme_preview <- function(base_size = 11) {
  ggplot2::theme_minimal(base_size = base_size) +
    ggplot2::theme(
      text = ggplot2::element_text(family = "sans", colour = "#25323c"),
      plot.background = ggplot2::element_rect(fill = "#fffdf8", colour = NA),
      panel.background = ggplot2::element_rect(fill = "#fffdf8", colour = NA),
      panel.grid.major = ggplot2::element_line(colour = "#dde4e2", linewidth = 0.35),
      panel.grid.minor = ggplot2::element_blank(),
      axis.title = ggplot2::element_blank(),
      axis.text = ggplot2::element_text(colour = "#66747b", size = 9),
...
bar_plot <- function(colors) {
  values <- c(8.7, 10.6, 9.3, 11.4, 12.2, 8.1, 10.9, 13.0)
  df <- data.frame(
...
make_plot_files <- function(id, colors) {
  ...
  vapply(names(plots), function(kind) {
    ...
    file.path("ocean_family_preview_figures", basename(out))
  }, character(1))
}
  ...
  sprintf(
    '<section class="palette-section">
      <div class="palette-head">
        <div>
          <p class="eyebrow">advanced family / ocean</p>
          <h2>%s <span>%s</span></h2>
          <p>%s</p>
        </div>
      </div>
...
<head>
  ...
  <title>ColorPhage Ocean Family Candidates</title>
  <style>
    :root {
      --bg: #eef3f1;
      --panel: #fffdf8;
    ...
    * { box-sizing: border-box; }
    body {
      ...
      color: var(--ink);
      background:
        radial-gradient(circle at 10%% 10%%, rgba(51,122,132,.18), transparent 28%%),
        radial-gradient(circle at 86%% 7%%, rgba(191,216,214,.34), transparent 24%%),
        radial-gradient(circle at 82%% 88%%, rgba(208,137,101,.14), transparent 23%%),
        var(--bg);
      line-height: 1.55;
    }
    ...
    .hero, .palette-section {
      ...
      border-radius: 30px;
      box-shadow: 0 22px 54px rgba(50,80,88,.10);
    }
...
<body>
  <main class="wrap">
    <section class="hero">
      <h1>ColorPhage Ocean Family Candidates</h1>
      <p>这是第三个进阶 family 的候选预览。Ocean 不做机械蓝色渐变，而是用海水青、深海蓝、冷白、雾蓝和少量暖色锚点，服务组学分层、空间图、连续结构和清洁型科研图。</p>
      <div class="mode-strip">
        <span class="pill">advanced family: ocean</span>
    ...
    %s
    <p class="footer">Generated by docs/design/build_ocean_family_candidates_preview.R</p>
  </main>
</body>
</html>',
  sections
...
out_file <- file.path(design_dir, "ocean_family_candidates_preview.html")
