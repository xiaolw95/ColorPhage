...
if (!requireNamespace("ggplot2", quietly = TRUE)) {
  stop("Package `ggplot2` is required to build the cinema candidate preview.", call. = FALSE)
}
...
palettes <- list(
  cinema_noir = list(
    ...
    role = "黑白电影骨架：适合疾病组/对照组核心结果、机制图关键通路和模型优劣对比。",
    colors = c("#1F2A36", "#3C4E63", "#6A7D91", "#A8B6C2", "#D9D5CC", "#C59A63", "#B45A53", "#7A5D78")
  ),
  cinema_marquee = list(
    display_name = "Cinema Marquee",
    role = "灯牌与舞台前景：适合主文摘要图、关键结果分组图和更明显锚点场景。",
    colors = c("#24313F", "#46606F", "#72959A", "#C1D2CF", "#E6DDD1", "#D8B06B", "#C97B5D", "#9A667C")
  ),
  cinema_velvet = list(
    display_name = "Cinema Velvet",
    role = "丝绒与剧场感：适合机制示意图、多亚型主结果图和叙事感更强的图。",
    colors = c("#2A2632", "#51455E", "#7B6B82", "#A9B0B9", "#D8D0C8", "#B89B6F", "#A95C58", "#5C7288")
  )
)

design_dir <- file.path(root_dir, "docs", "design")
figure_dir <- file.path(design_dir, "cinema_family_preview_figures")
...
theme_preview <- function(base_size = 11) {
  ggplot2::theme_minimal(base_size = base_size) +
    ggplot2::theme(
      text = ggplot2::element_text(family = "sans", colour = "#2d2c31"),
      plot.background = ggplot2::element_rect(fill = "#fffaf2", colour = NA),
      panel.background = ggplot2::element_rect(fill = "#fffaf2", colour = NA),
      panel.grid.major = ggplot2::element_line(colour = "#e4ddd3", linewidth = 0.35),
      panel.grid.minor = ggplot2::element_blank(),
      axis.title = ggplot2::element_blank(),
      axis.text = ggplot2::element_text(colour = "#736d68", size = 9),
...
bar_plot <- function(colors) {
  values <- c(8.9, 11.4, 9.8, 12.6, 10.7, 8.2, 13.0, 10.5)
...
make_plot_files <- function(id, colors) {
  ...
  vapply(names(plots), function(kind) {
    ...
    file.path("cinema_family_preview_figures", basename(out))
  }, character(1))
}

swatch_cards <- function(colors) {
  paste(sprintf('<div class="swatch-card"><div class="chip" style="background:%s"></div><code>%s</code></div>', colors, colors), collapse = "\n")
}

plot_card <- function(title, src) {
  sprintf('<div class="preview-card"><h3>%s</h3><img src="%s" alt="%s"></div>', escape_html(title), escape_html(src), escape_html(title))
}
...
palette_section <- function(id) {
  ...
  cards <- paste(
    plot_card("Fill / bar", files[["bar"]]),
    plot_card("Colour / scatter", files[["scatter"]]),
    plot_card("Colour / line", files[["line"]]),
    plot_card("UMAP-like", files[["umap"]]),
    sep = "\n"
  )
  sprintf(
    '<section class="palette-section"><div class="palette-head"><div><p class="eyebrow">advanced family / cinema</p><h2>%s <span>%s</span></h2><p>%s</p></div></div><div class="swatches">%s</div><div class="preview-grid">%s</div></section>',
    escape_html(meta$display_name), escape_html(id), escape_html(meta$role), swatch_cards(meta$colors), cards
  )
}
...
<head>
  ...
  <title>ColorPhage Cinema Family Candidates</title>
  <style>
    :root { --bg: #f5efe7; --panel: #fffaf2; --ink: #2d2c31; --muted: #736d68; --line: #ddd4c8; }
    * { box-sizing: border-box; }
    body { margin: 0; font-family: "Segoe UI", "PingFang SC", "Microsoft YaHei", sans-serif; color: var(--ink); background: radial-gradient(circle at 10%% 10%%, rgba(122,93,120,.16), transparent 26%%), radial-gradient(circle at 86%% 6%%, rgba(197,154,99,.20), transparent 24%%), radial-gradient(circle at 78%% 88%%, rgba(60,78,99,.16), transparent 24%%), var(--bg); line-height: 1.55; }
    .wrap { width: min(1280px, calc(100%% - 48px)); margin: 36px auto 72px; }
    .hero, .palette-section { background: rgba(255,250,242,.94); border: 1px solid var(--line); border-radius: 30px; box-shadow: 0 22px 54px rgba(80,68,62,.10); }
    ...
    .pill { border: 1px solid var(--line); border-radius: 999px; padding: 8px 14px; background: #fbf4ea; color: var(--muted); font-size: 14px; }
    ...
    .chip { height: 98px; }
    .swatch-card code { display: block; padding: 10px 12px 12px; font-size: 12px; color: var(--muted); background: #fbf4ea; }
    .preview-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 14px; }
    .preview-card { border: 1px solid var(--line); background: linear-gradient(180deg, #fff, #fbf4ea); border-radius: 22px; padding: 16px 16px 12px; overflow: hidden; }
    .preview-card h3 { font-size: 15px; color: var(--muted); margin-bottom: 10px; }
    .preview-card img { width: 100%%; height: auto; display: block; border-radius: 14px; background: #fffaf2; }
...
<body>
  <main class="wrap">
    <section class="hero">
      <h1>ColorPhage Cinema Family Candidates</h1>
      <p>这是第五个进阶 family 的候选预览。Cinema 用深色骨架、暖金属锚点和受控红褐来强化主结果聚焦感，让图像更像“有镜头语言”，但仍保持科研发表友好。</p>
      <div class="mode-strip">
        <span class="pill">advanced family: cinema</span>
    ...
    %s
    <p class="footer">Generated by docs/design/build_cinema_family_candidates_preview.R</p>
  </main>
</body>
</html>',
  sections
...
out_file <- file.path(design_dir, "cinema_family_candidates_preview.html")
