...
- Built the first discrete palette system with 24 confirmed palettes across
  ...
  `mist`.
- Added `phage_palette()`, `available_phage_palettes()`,
  `scale_color_phage()`, and `scale_fill_phage()`.
- Added `theme_phage()` for publication-style ggplot2 output.
- Added structured `n = 10 / 12` support for the eight default palettes.
- Added experimental high-cardinality helpers:
